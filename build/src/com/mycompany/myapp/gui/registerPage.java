...
public class registerPage extends BaseForm{
    ...
    public registerPage(Resources res) {
        ...
        TextField tfroles = new TextField("", "role", 20, TextField.ANY);
     // Label tfroles = new Label("Role");
        //tfroles.setUIID("Label");
        ComboBox Cg = new ComboBox<>();   
     //   addStringValue("Profile picture", FlowLayout.encloseRightMiddle(Cg.getSelectedItem().toString()));
        String[] lesTYpes = {"Client", "Provider"};
        for (int i = 0; i < 2; i++) {
        ...
        Label alreadHaveAnAccount = new Label("Already have an account?");
            ...
            public void actionPerformed(ActionEvent evt) {
                if ((tfnomuser.getText().length()==0)||(tfemail.getText().length()==0)||(tfpassword.getText().length()==0)||( Cg.getSelectedItem().toString().isEmpty()))
                ...
                else
                {
                    try {
                        ...
                        if (Cg.getSelectedItem().toString().equals("Client"))
                        {u.setRoles("ROLE_CLIENT"); }
                        else {u.setRoles("ROLE_FOUR"); }
